  ...
  getAllAromaBeer: async (req, res) => {
    ...
    if (!page || !size) {
      return res
        ...
        .send(
          util.fail(statusCode.BAD_REQUEST, responseMessage.NO_PAGE_OR_SIZE)
        );
    }

    const { limit, offset } = await informationService.get_pagination(
      page,
      size
    );
    try {
      ...
      const result = await informationService.get_paging_data(
        beers,
        page,
        limit
      );
      ...
      return res
        ...
        .send(
          util.fail(
            ...
            responseMessage.BEER_READ_ALL_FAIL
          )
        );
    }
  },
