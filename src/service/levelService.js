...
module.exports = {
  calc_user_review_level: async (review_count, req, res) => {
    try {
      let level_id = 0;
      if (review_count < 5) return (level_id = 1);
      else if (review_count < 25) return (level_id = 2);
      else if (review_count < 100) return (level_id = 3);
      else if (review_count < 500) return (level_id = 4);
      else return (level_id = 5);
    } catch (error) {
      console.log(error);
  ...
  calc_user_review_and_level: async (user_id, req, res) => {
    try {
      ...
      //유저 다음 레벨 계산
      const next_level_data = levels.find(
        (level_data) => Number(level_data.id) === user.level_id + 1
      );
      //남은 리뷰 갯수 계산
      ...
      user_result_data.next_level_id = next_level_data.id;
      user_result_data.need_review_count = need_review_count;
      user_result_data.next_level = next_level_data.level;
      user_result_data.next_level_image = next_level_data.level_image;
