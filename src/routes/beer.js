...
/**
 * @api {get} /api/style:num
 * @apiName product/style
 * @apiGroup Water
 */
...
router.get('/', async (req, res) => {
  ...
  try {
    const beer = await Beer.findAll({
      ...
      // status: 200, message: READ_USER_ALL_SUCCESS, result: 반환
    })
  } catch (error) {
    console.error(error);
...
router.get('/', async (req, res) => {
  try {
    ...
    console.error(error);
    return res.status(statusCode.INTERNAL_SERVER_ERROR).send(util.fail(statusCode.INTERNAL_SERVER_ERROR, responseMessage.NO_BEER_ID));
  }
});
/*
